...
let kopayServerEnv = {};
fs.readFileSync('../server/env/production', 'utf8').replace(/(\w+)=(.+)/g, function($0, $1, $2) { kopayServerEnv[$1] = $2; });
...
let ghostServerEnv = {}
fs.readFileSync('../ghost/env/production', 'utf8').replace(/(\w+)=(.+)/g, function($0, $1, $2) { ghostServerEnv[$1] = $2; });

//Ghost requires a static url
//Be sure that if https in kopayServerAdress is also enabled on this url
const kopayURL = (ghostServerEnv && ghostServerEnv.url ) ? ghostServerEnv.url : 
( kopayServerEnv && kopayServerEnv.HTTPS) ? 'https://www.kopay.com.mx' : 'http://www.kopay.com.mx'

let kopayClientEnv = {};
fs.readFileSync('../client/env/production', 'utf8').replace(/(\w+)=(.+)/g, function($0, $1, $2) { kopayClientEnv[$1] = $2; });

const GLOBALS = {
  'process.env.NODE_ENV': JSON.stringify('production'),
  'process.env.KOPAY_SERVER_ADDRESS': JSON.stringify(kopayServerAddress),
  'process.env.KOPAY_URL': JSON.stringify(kopayURL),
