...
// Actions
import { getProductGroup, clearVariableObjectData } from '../../../actions/variableObjectActions';
import { addProductsToProductGroup, removeProductsFromProductGroup } from '../../../actions/adminProductGroupsActions';
...
class ProductGroupProductsModal extends Component {
    constructor(props) {
        super(props);
    ...
    componentWillMount() {
        ...
        const loadPromises = [
            this.props.getProductGroup(productGroupID),
        ...
        Promise.all(loadPromises).then(() => {
            ...
            const rightPagerItemCount = productGroup.products && productGroup.products.length || 0;
    ...
    setOrGetTransferData({ returnState, props }) {
        ...
        const R_Col_Items_Paginated = productGroupProducts.filter(p => R_Col_Keys_paginated.find(p2 => p2 === p._id) !== undefined)
            .map(p => this.mapProductToTransferItem(p));

        // From all products, filter the ones that are NOT in the right column
        const L_Col_items_all = allProducts.filter(p => R_Col_keys_all.find(key => key === p._id) === undefined);
    ...
    onItemTransfer(newRightColumnKeys, direction, movedKeys) {
        const { productGroupID, allProductsOwnPager } = this.props;

        // Adding to productGroup.products
        if (direction === 'right') {
            this.setState({ loading: true }, () => {
                this.props.goToPage(allProductsOwnPager, 1);
                this.props.addProductsToProductGroup(productGroupID, movedKeys).finally(() => {
                    this.setState({ loading: false });
                });
            });
        } else if (direction === 'left') { // Removing from productGroup.products
            this.setState({ loading: true }, () => {
                this.props.removeProductsFromProductGroup(productGroupID, movedKeys).finally(() => {
                    this.setState({ loading: false });
                });
            });
        }
    ...
    onAllProductsFilterChange(filter) {
        const { allProductsServerPager, allProductsOwnPager } = this.props;
    ...
    render() {
        ...
        const rightColumnTitle = (
            <div className="transfer-column-title">
                ...
                <span className="title">Productos en el grupo</span>
            </div>
        );

        return (
            <Modal title="ProductGroup Products"
                visible={true}
                onCancel={closeFn}
                maskClosable={true}
                footer={null}
...
ProductGroupProductsModal.propTypes = {
    ...
    getProductGroup: PropTypes.func.isRequired,
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        getProductGroup: (productGroupID) => {
            return dispatch(getProductGroup(productGroupID, false));
        },
...
export default connect(mapStateToProps, mapDispatchToProps)(ProductGroupProductsModal);